...
def client():
    ...
    try:
        ...
        clientSocket.send(encryptMessage(sym_key, "sync message"))
        
        #receive nonce
        nonce = clientSocket.recv(2048)
        #nonce = decryptMessage(sym_key, encryptedNonce)
        #encryptSendBackNonce = encryptMessage(sym_key, nonce)
        clientSocket.send(nonce)
        clientSocket.recv(2048)#sync message
