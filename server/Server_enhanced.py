        ...
        try:
            ...
            if id == 0: #child process
                ...
                try:
                    ...
                    connectionSocket.recv(2048) #sync message
                    #send a signature nonce to check if what is returned it the same
                    nonce = get_random_bytes(int(256/8))
                    #encrypted_nonce = encryptMessage(sym_key, nonce)
                    connectionSocket.send(nonce)
                    
                    clientNonce= connectionSocket.recv(2048)
                    #clientNonce = decryptMessage(encryptedClientNonce)
                    if clientNonce != nonce:
                        print("Hello hacker, I'm calling the FBI")
